...
| :--- | :--- | :--- | :--- | :--- |
| **SVC-STK-001** | **정상 처리**<br>(Redis/DB 저장 성공) | 1. 유효한 `Event` 객체<br>2. `MetadataProvider`: 종목명 반환<br>3. `Mapper`: Entity 변환 성공 | `processRealTimeTrade(event)` 호출 | 1. `MetadataProvider.getStockName()` 호출<br>2. `StockSnapshotRepository.save()` 1회 호출<br>3. `StockHistoryRepository.save()` 1회 호출 |
...
| :--- | :---: | :---: | :---: | :--- |
| **Real-Time Process** | 4 | 4 | 0 | 데이터 파이프라인(Event -> Redis/DB) 및 Validation 검증 완료 |
