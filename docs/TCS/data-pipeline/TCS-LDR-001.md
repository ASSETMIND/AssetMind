...
1. **초기화 및 자원 방어:** 2건 (정상 생성, Config 누락 방어)
...
| :-----------: | :--: | :---: | :----------------------------------------------------------------------- | :------------------------------------------- | :----------------------------------------------------------------------------- | :---------------------------------------- |
|  **INIT-01**  | 단위 |  BVA  | `ConfigManager` 인스턴스 대신 `None`이 주어짐                            | `AbstractLoader`를 상속받은 Mock 로더 초기화 | `ConfigurationError` 예외가 발생해야 함                                        | `config=None`                             |
|  **INIT-02**  | 단위 | 표준  | 정상적인 `ConfigManager` 인스턴스가 주어짐                               | Mock 로더 초기화                             | 에러 없이 객체가 생성되고 로거가 초기화되어야 함                               | `config=MockConfig()`                     |
|  **LOAD-01**  | 단위 | 표준  | `_validate_dto`가 `True`, `_apply_load`가 `True`를 반환하도록 Mocking됨  | `load(dto)` 호출                             | 예외 없이 `True`를 반환해야 함                                                 | `dto=MockDTO()`                           |
|  **LOAD-02**  | 단위 | 표준  | `_validate_dto`는 `True`, `_apply_load`는 `False`를 반환하도록 Mocking됨 | `load(dto)` 호출                             | 예외 없이 `False`를 반환해야 함                                                | `dto=MockDTO()`                           |
| **FAIL-V-01** | 단위 | MC/DC | `_validate_dto` 로직이 `False`를 반환하도록 설정됨                       | `load(dto)` 호출                             | 1. `LoaderValidationError` 발생<br>2. `_apply_load`는 호출되지 않음            | `dto=InvalidDTO()`                        |
| **FAIL-T-01** | 단위 | 방어  | `_apply_load`가 `bool`이 아닌 값(`str`, `None` 등)을 반환하도록 설정됨   | `load(dto)` 호출                             | 반환 타입 에러가 감지되어 `LoaderError` 예외가 발생해야 함                     | Return: `"Success"`                       |
...
|  **ABST-01**  | 단위 | 구조  | 추상 클래스의 원본 메서드(`_validate_dto`, `_apply_load`)에 접근         | 원본 추상 메서드를 직접 호출                 | 예외 발생 없이 내부의 `pass` 블록을 통과해야 함                                | `AbstractLoader._validate_dto(mock, dto)` |
