...
| **TC-013** | Concurrency | 토큰이 만료되었거나 없는 상태에서 다수의 요청이 동시에 유입됨 (Race Condition) | `asyncio.gather`를 사용하여 `get_token` 동시 호출 | 1. 모든 요청이 정상적으로 토큰을 획득함<br>2. 실제 API 호출은 **단 1회**만 발생해야 함 (Lock 동작 검증) | Concurrent Requests (N=5)<br>Mock Latency (0.1s)                                             |    P1    |
