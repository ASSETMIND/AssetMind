...
데이터 흐름 (Data Flow):
Input (Task Name) -> Load .env & configs/{task_name}.yml -> Pydantic Parsing -> ConfigManager Object
...
class ConfigManager(BaseSettings):
    ...
    @classmethod
    def load(cls, task_name: str) -> "ConfigManager":
        ...
        Returns:
            ConfigManager: 초기화 및 검증이 완료된 설정 객체.
        """
        config = cls()
...
def get_config(task_name: Optional[str] = None) -> ConfigManager:
    ...
    if task_name not in _config_cache:
        # Lazy Loading 수행
        _config_cache[task_name] = ConfigManager.load(task_name)
