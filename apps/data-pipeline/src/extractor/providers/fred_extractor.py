...
class FREDExtractor(AbstractExtractor):
    ...
    def _create_response(self, raw_data: Any, job_id: str) -> ExtractedDTO:
        ...
        """
        # [설계 의도] FRED API는 잘못된 쿼리 등 논리적 오류가 발생해도 HTTP 200 OK와 함께 
        # Body 내부에 에러 페이로드(예: {"error_code": 400, "error_message": "..."})를 
        # 반환하는 패턴이 있으므로, 데이터 적재 전 이를 필터링하는 방어적 로직 적용.
        if "error_message" in raw_data:
            msg = raw_data.get("error_message")
            code = raw_data.get("error_code", "알 수 없는 코드")
            raise ExtractorError(f"FRED API 실패: {msg} (Code: {code})")

        # [설계 의도] 모든 검증을 통과한 순수 데이터에 한해 파이프라인 
        # 다음 단계(Loader)로 넘기기 위해 표준 컨트랙트(ExtractedDTO)로 래핑함.
        return ExtractedDTO(
            data=raw_data,
            meta={
                "source": "FRED",
