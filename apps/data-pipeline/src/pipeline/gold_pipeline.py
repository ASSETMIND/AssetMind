...
class GoldPipeline(AbstractPipeline):
    ...
    async def run_batch(self, execution_date: Optional[str] = None, extract_mode: str = "TODAY") -> Dict[str, Any]:
        ...
        self._summary_registry = {
            ...
            "status": "PROCESSING",
            "reader": {"collected_days": 0, "status": "PENDING"},
            "preprocessor": {"status": "PENDING", "buckets_count": 0},
            "loader": {"status": "PENDING", "success": 0, "fail": 0}
        }
        ...
        try:
            ...
            for i in range(max_search_days):
                ...
                source_job_id = self._task_name.replace("gold_", "silver_")
                try:
                    # [설계 의도] ReaderService의 스트리밍 판독기 인터페이스 계약에 맞춰 task_name을 job_id로 매핑하여 호출
                    raw_data_stream = self._reader_service.read_stream(
                        job_id=source_job_id,  # self._task_name에서 source_job_id로 교정
                        execution_date=search_date_str,
                        source_layer="silver"
                    )
                    
                    job_chunks = [pd.DataFrame(chunk) for chunk in raw_data_stream if chunk]
                    if job_chunks:
                        ...
                        if not day_df.empty:
                            ...
                            self._logger.info(
                                f"[{self._task_name}] {search_date_str} 실버 데이터 로드 성공 "
                            )
                    ...
                    self._logger.warning(
                        f"[{self._task_name}] {search_date_str} 실버 데이터 로드 중 스킵 발생 (사유: {str(e)})"
                    )
                    continue

            # [설계 의도] 신규 자산이거나 백필 초기 단계여서 20거래일 미만으로 조회되더라도 가용한 일부 데이터만이라도 바인딩하여 진행
            if not collected_dfs:
                self._summary_registry["status"] = STATUS_EMPTY
                self._summary_registry["reader"]["status"] = "EMPTY"
                self._summary_registry["preprocessor"]["status"] = "SKIPPED"
                self._summary_registry["loader"]["status"] = "SKIPPED"
                self._log_pipeline_summary()  # 진단 불능 상태를 타파하기 위한 스킵 리포트 강제 인쇄
            ...
            self._summary_registry["reader"]["collected_days"] = lookback_days
            self._summary_registry["reader"]["status"] = STATUS_SUCCESS
            ...
            # [Cold Start 허용] 20일 미만 유입 시 조기 종료(return)하지 않고 Cold Start 모드로 전처리 레이어에 진입
            if lookback_days < 20:
                self._summary_registry["reader"]["status"] = "INSUFFICIENT_WINDOW"
                self._logger.warning(
                    f"[{self._task_name}] 확보된 거래일수({lookback_days}일)가 기준 윈도우(20일)보다 부족합니다. "
                    f"Cold Start 모드로 전처리 서비스에 진입합니다."
                )
            else:
                self._summary_registry["reader"]["status"] = STATUS_SUCCESS
            ...
            try:
                ...
                # 전처리 계층 가시성 메트릭 확보
                self._summary_registry["preprocessor"]["status"] = STATUS_SUCCESS
                self._summary_registry["preprocessor"]["buckets_count"] = len([k for k in final_artifacts.keys() if k.startswith("bucket_")])
                
            except PreprocessorError as pe:
                self._summary_registry["status"] = STATUS_FAIL_PROCESS
                self._summary_registry["preprocessor"]["status"] = "FAILED"
                self._summary_registry["loader"]["status"] = "SKIPPED"
                self._log_pipeline_summary()  # 연산 패닉 분기점 강제 사출
            ...
            for artifact_key, artifact_df in final_artifacts.items():
                ...
                try:
                    # [year, month, day] 파티션 컬럼 자동 생성 및 주입
                    export_df = artifact_df.copy()
                    if isinstance(export_df.index, pd.DatetimeIndex) and not export_df.empty:
                        export_df["year"] = export_df.index.year.astype(str)
                        export_df["month"] = export_df.index.strftime("%m")
                        export_df["day"] = export_df.index.strftime("%d")
                    else:
                        dt = pd.to_datetime(target_date_str)
                        export_df["year"] = dt.strftime("%Y")
                        export_df["month"] = dt.strftime("%m")
                        export_df["day"] = dt.strftime("%d")
            ...
            self._summary_registry["loader"]["success"] = success_count
            self._summary_registry["loader"]["fail"] = fail_count
            
            if fail_count > 0:
                self._summary_registry["status"] = STATUS_FAIL_PROCESS
                self._summary_registry["loader"]["status"] = "PARTIAL_FAILED"
                self._log_pipeline_summary()  # 일부 실패 지점 리포트 사출
            ...
            # 전체 파이프라인 무결 정산 완결
            self._summary_registry["status"] = STATUS_SUCCESS
            self._summary_registry["loader"]["status"] = STATUS_SUCCESS
            self._log_pipeline_summary()  # 최종 퍼펙트 통과 리포트 사출
            ...
            self._summary_registry["status"] = STATUS_SYSTEM_ERROR
            if "reader" in self._summary_registry and self._summary_registry["reader"]["status"] == "PENDING":
                self._summary_registry["reader"]["status"] = "CRASHED"
            if "preprocessor" in self._summary_registry and self._summary_registry["preprocessor"]["status"] == "PENDING":
                self._summary_registry["preprocessor"]["status"] = "CRASHED"
            if "loader" in self._summary_registry and self._summary_registry["loader"]["status"] == "PENDING":
                self._summary_registry["loader"]["status"] = "CRASHED"
            self._log_pipeline_summary()  # 예측 불허 인프라/런타임 패닉 독약 리포트 사출
            
            self._logger.error(f"[{self._task_name}] Gold 오케스트레이터 내부에서 예측하지 못한 치명적 시스템 에러 발생: {e}")
    ...
    def _log_pipeline_summary(self) -> None:
        """[설계 의도] 파이프라인의 임의 탈출 지점(공백 스킵, 에러 크래시, 정상 종료)에 관계없이 
        3대 하위 서비스(Reader/Preprocessor/Loader)의 핵심 통계 리포트를 가이드라인 포맷으로 로그에 인쇄합니다.
        """
        summary_bar = "=" * 98
        self._logger.info(
            f"\n{summary_bar}\n"
            f"              [GOLD PIPELINE BATCH SUMMARY REPORT - TASK: {self._task_name}]\n"
            f"{summary_bar}\n"
            f"■ BATCH EXECUTION CONTEXT\n"
            f"  - Execution Date   : {self._summary_registry['execution_date']}\n"
            f"  - Target Trade Date: {self._summary_registry['target_date']}\n"
            f"  - Final Job Status : {self._summary_registry['status']}\n\n"
            f"■ 1. READER SERVICE SUMMARY\n"
            f"  - Source Layer     : Silver Layer (S3 Parquet)\n"
            f"  - Requested Window : 20 Trading Days\n"
            f"  - Collected Days   : {self._summary_registry['reader']['collected_days']} / 20 Days\n"
            f"  - Data Density     : {self._summary_registry['reader']['status']}\n\n"
            f"■ 2. PREPROCESSOR SERVICE SUMMARY\n"
            f"  - Service Status   : {self._summary_registry['preprocessor']['status']}\n"
            f"  - 사출된 실험 버킷  : {self._summary_registry['preprocessor']['buckets_count']} / 18 Buckets\n\n"
            f"■ 3. LOADER SERVICE SUMMARY\n"
            f"  - Target Storage   : S3 Parquet (Gold Layer)\n"
            f"  - Load Success     : {self._summary_registry['loader']['success']} Buckets\n"
            f"  - Load Failure     : {self._summary_registry['loader']['fail']} Buckets\n"
            f"{summary_bar}"
        )