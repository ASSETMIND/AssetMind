...
class PipelineService:
    ...
    async def run_batch(self, execution_date: Optional[str] = None, extract_mode: str = "TODAY") -> Dict[str, Any]:
        ...
        Airflow 기반의 멱등성(Idempotency) 보장과 백필(Backfill)을 지원하기 위해, 
        스케줄러로부터 논리적 실행일(`execution_date`)과 실행 모드(`extract_mode`)를 주입받는 인터페이스로 확장함.
        ...
        # 런타임 환경 구성
        # 스케줄러가 명시하지 않은 경우에만 로컬 시간으로 Fallback(방어적 프로그래밍)
        if not execution_date:
            execution_date = datetime.now().strftime("%Y%m%d")

        runtime_params = {
            "EXECUTION_DATE": execution_date,
            "EXTRACT_MODE": extract_mode.upper()
        }

        # 단순 문자열 리스트가 아닌 (job_id, runtime_params) 튜플 리스트로 매핑하여 하위 위임
