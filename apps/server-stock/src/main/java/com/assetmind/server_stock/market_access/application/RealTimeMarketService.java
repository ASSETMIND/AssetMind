...
public class RealTimeMarketService {
    ...
    // 테스트용 주식 코드
    // 추후 DB에서 꺼내와 유동적으로 주식 데이터를 가져오도록 구현할 예정
    private List<String> stockCodes = List.of("005930","000660","035420");
    ...
    public void startMarketDataCollection() {
        log.info(">>> [System] 실시간 주식 데이터 수집 서비스를 시작합니다.");
        log.info(">>> 구독 대상 종목: {}", stockCodes);

        try {
            // 1. 인증키 가져오기 (이미 구현된 로직 활용)
            ...
            log.error(">>> [System] 초기화 중 치명적 오류 발생", e);
            // 필요 시 알림 전송 로직 추가
        }
    }
    ...
    public void stopMarketDataCollection() {
        log.warn(">>> [System] 서버 종료 감지. 연결을 해제합니다.");
